...
\SwedishKeywords{Flödesbaserad marknadskoppling, ATC-kapacitet, Nordisk elmarknad, Effektöverföringsfaktorer, Trängselhantering, Zonprissättning, Marknadseffektivitet}
...
\begin{scontents}[store-env=abstracts,print-env=true]
Den europeiska elmarknaden genomgår en genomgripande förändring i hur gränsöverskridande överföringskapacitet allokeras, där flödesbaserad marknadskoppling (FBMC) ersätter den traditionella metoden med tillgänglig överföringskapacitet (ATC). Detta examensarbete presenterar en kvantitativ jämförelse mellan FBMC och ATC för den nordiska elmarknaden, med särskilt fokus på det svenska transmissionssystemet efter FBMC-implementeringen i oktober 2024.
...
Resultaten visar att FBMC ger mätbara effektivitetsvinster jämfört med ATC, men med betydande säsongsvariation. Under vinterförhållanden (december 2024) uppnådde FBMC 6,5\% kostnadsbesparingar (4,37 miljoner EUR per vecka) jämfört med ATC, drivet av hög trängsel på SE2--SE3-korridoren (88\% utnyttjande). Under sommarförhållanden (juli 2024) var besparingarna endast 0,25\% på grund av minskad trängsel (53\% utnyttjande), vilket motsvarar en cirka 50-faldig skillnad mellan säsongerna. Analysen använder ett proportionellt antagande för generatorskiftnycklar (GSK); alternativa GSK-metoder kan ge andra absoluta storleksordningar (litteraturen antyder $\pm$10--20\% variation) samtidigt som det riktningsberoende resultatet---att FBMC överträffar ATC under trängselförhållanden---bevaras.

Känslighetsanalys över fem scenarier bekräftade robustheten i dessa resultat: FBMC-fördelarna ökar vid begränsad transmission (16,1\% besparing vid 30\% SE2--SE3-kapacitetsreduktion) och försvinner helt vid priskonvergens, vilket validerar att mekanismen fungerar genom förbättrad arbitragefacilitering. Experiment med korridorisolering kvantifierade de underliggande mekanismerna: cirka 32\% av FBMC-fördelarna uppstår från utnyttjande av kretskraftsflöden genom norska sammanlänkningar, medan 68\% härrör från förbättrad optimering av Sveriges interna korridorer. Dessa resultat visar att FBMC levererar störst värde under perioder med hög trängsel när transmissionsbegränsningar aktivt binder.
...
I would like to express my sincere gratitude to my supervisor, Saeed Mohammadi, and my examiner, Professor Mohammad Reza Hesamzadeh for their guidance, expertise and support throughout this project. 
...
The efficient allocation of cross-border transmission capacity is fundamental to the operation of interconnected electricity markets. As European power systems become increasingly integrated, the methodology used for capacity allocation directly impacts market efficiency, price formation, and ultimately consumer welfare~\cite{kirschen_strbac_2018}. This chapter introduces the context, problem statement, and objectives of this thesis.
...
The fundamental limitation of ATC lies in its treatment of the physical network. By reducing the complex, meshed transmission grid to simple bilateral capacity limits, ATC cannot capture the physical reality that power flows distribute across multiple parallel paths according to Kirchhoff's laws~\cite{wood_wollenberg_2014}. In meshed networks, a transaction between zones A and B will cause flows not only on the direct A-B corridor but also on alternative paths through intermediate zones---so-called loop flows. The ATC methodology must embed conservative margins to account for these effects, resulting in systematic underutilization of actual physical capacity.
...
The \gls{CWE} region pioneered FBMC implementation in May 2015, with subsequent analyses documenting welfare gains of 150--300 million EUR annually~\cite{acer_fbmc_2019}. Building on this experience, the Nordic electricity market---comprising Sweden, Norway, Finland, and Denmark---completed its transition to FBMC in October 2024~\cite{entsoe_nordic_ccm_2022} following an 18-month parallel operation period coordinated by the Nordic \gls{RCC}.

\section{Problem Definition}
\label{sec:problem}

The Swedish transmission system presents a compelling case study for analyzing FBMC effectiveness. Sweden's four bidding zones (SE1--SE4) exhibit a predominantly north-south radial topology, with the SE2--SE3 corridor representing the primary bottleneck constraining power transfer from hydroelectric-dominated northern regions to load-intensive southern consumption centers. This structural characteristic raises fundamental questions regarding the magnitude of efficiency gains achievable through FBMC in systems with limited internal loop flow opportunities.
...
Sweden's internal transmission system follows a predominantly north-south orientation, reflecting the geographic separation between hydroelectric generation in the north and population centers in the south. The SE2--SE3 corridor, with a nominal NTC of approximately 7,300~MW~\cite{svk_perspektivplan_2024}, represents the critical bottleneck constraining power transfer from the generation-rich northern zones to load-intensive SE3 and SE4. This corridor's frequent congestion makes it a natural focus for analyzing capacity allocation methodologies.
...
The Central Western European (CWE) implementation, operational since May 2015, provides the most extensive empirical evidence on FBMC performance. Post-implementation analyses by ACER and the CWE TSOs documented welfare gains of 150--300 million EUR annually~\cite{cwe_tsos_2020, cwe_report_2020}, though these estimates vary depending on the counterfactual ATC assumptions employed. Notably, the CWE experience revealed implementation challenges including the need for frequent PTDF matrix updates, coordination across multiple TSOs, and market participant adaptation to more complex price formation dynamics.
...
\hspace{1.5em}\textbf{Power Balance:}
\begin{equation}
...
The proportional GSK assumption represents a methodological choice with quantifiable impacts on results. Schönheit et al.~\cite{schonheit2020gsk} demonstrated that alternative GSK strategies can yield PTDF variations of 10--20\% on critical network elements. This suggests the absolute magnitude of benefits reported in this thesis (e.g., 6.5\% winter savings) should be interpreted as indicative estimates subject to GSK-dependent uncertainty, while the directional findings---FBMC outperforms ATC, benefits increase with congestion, benefits concentrate in winter---are expected to be robust across GSK methodologies. Future work should validate these findings using the operational GSK strategies employed by Nordic TSOs post-implementation.
...
To capture seasonal variation and validate the robustness of base case findings, complete 168-hour market simulations were conducted for three representative periods in 2024:

\begin{itemize}
    \item \textbf{January 2024} (weeks 2--3): Winter peak demand conditions with average hourly demand of 20,094 MW
    \item \textbf{December 2024} (weeks 49--50): Winter high-congestion period with average hourly demand of 17,500 MW
    \item \textbf{July 2024} (weeks 28--29): Summer low-demand conditions with average hourly demand of 10,847 MW
\end{itemize}

Each period simulation employed period-specific input data from authoritative sources: hourly demand profiles from the ENTSO-E Transparency Platform, historical day-ahead prices for neighboring zones, and generation availability by technology type. Network topology and NTC values remained constant across periods to isolate the effect of varying demand and price conditions. This approach ensures that seasonal comparisons reflect actual variations in system conditions rather than extrapolations from a single base case.
...
\begin{itemize}
    ...
    \item \texttt{HiGHS}: High-performance LP solver~\cite{huangfu_2018} (via scipy)
\end{itemize}
...
\begin{table}[ht]
\centering
\caption{Base Case Results: December 2024. Note: Negative cost values reflect the model's treatment of fixed demand as negative generation with associated consumer welfare; FBMC's less negative value indicates higher total welfare.}
\label{tab:base_case}
\begin{tabular}{lrr}
\hline
...
Weekly Savings (M EUR) & 4.37 & \\
Percentage Savings & 6.5\% & \\
SE2--SE3 Utilization & 88\% & 99\% \\
Average SE3 Price (EUR/MWh) & 39.1 & 42.1 \\
\hline
\end{tabular}
\end{table}
...
A & Continental prices +30\% & 4.9 & 4.41 \\
B & Norwegian prices -30\% & 4.6 & 3.00 \\
...
\hline
January 2024 & 12.6\% & 91\% & 13.8 EUR/MWh \\
December 2024 & 6.5\% & 88\% & 10.0 EUR/MWh \\
July 2024 & 0.25\% & 53\% & 9.8 EUR/MWh \\
\hline
\end{tabular}
\end{table}
...
To quantify the loop flow contribution, a corridor isolation experiment was conducted using the January 2024 high-congestion period. The simulation was re-run with Norwegian interconnections (SE1--NO4, SE2--NO3, SE3--NO1) removed from the network model. Under full network conditions, FBMC achieved 12.6\% cost savings (6.36M EUR weekly) compared to ATC. With Norwegian connections removed, FBMC savings decreased to 10.7\% (4.31M EUR weekly)---a reduction of 1.9 percentage points. This indicates that loop flows through Norwegian interconnections account for approximately 32\% of observed FBMC benefits (2.05M EUR), while the remaining 68\% (4.31M EUR) arises from improved optimization of Sweden's internal corridors even without cross-border alternatives.

Flow analysis confirms this routing mechanism. Under FBMC, power flows counterclockwise through the Norwegian network: Sweden exports to Norway via SE1--NO4 (-600~MW average) and SE2--NO3 (-937~MW), while Norway returns power to southern Sweden via NO1--SE3 (+1410~MW). Under ATC, these flows are reversed---Sweden imports from Norway in the north and exports in the south---because ATC treats each corridor independently and cannot coordinate the loop. This flow reversal explains the reduced SE2--SE3 utilization under FBMC (88\% versus 99.7\%): approximately 1,600~MW is routed through the Norwegian loop rather than the direct Swedish corridor.
...
The second hypothesis concerned topology dependency---specifically, whether FBMC benefits require network loops to materialize. The corridor isolation experiment provides quantitative evidence on this hypothesis. Contrary to the initial expectation that loop flows would be the dominant mechanism, the results reveal a more nuanced picture: approximately 32\% of FBMC benefits arise from loop flow exploitation through Norwegian interconnections, while 68\% derive from improved utilization of Sweden's internal transmission corridors. This finding suggests that FBMC delivers value through two complementary mechanisms---exploiting parallel cross-border paths when available, and optimizing flow allocation across internal corridors even in predominantly radial networks. The presence of cross-border connections amplifies FBMC benefits but is not a prerequisite for positive welfare effects.
...
The remaining discrepancy between simulated and official estimates likely reflects several factors: static NTC values that may be more conservative than operational TSO practice, the omission of N-1 contingency CNECs beyond the 13 corridors modeled, the 18-month official averaging period versus representative weeks analyzed here, and strategic bidding behavior not captured under competitive market assumptions.
...
Operational FBMC implementations consider hundreds of N-1 contingencies through CNEC selection, monitoring how line flows would redistribute following the outage of any single critical element. This thesis employs 13 static corridor limits without explicit contingency analysis. This simplification likely understates FBMC benefits, as ATC methodologies are particularly conservative under contingency scenarios where the physical flexibility to redistribute flows would be most valuable.

This analysis assumes both methodologies operate without computational or coordination failures. Operational FBMC implementation introduces complexity not captured in this model: real-time PTDF matrix updates, multi-TSO coordination requirements, and solver performance under time constraints. The Nordic RCC's successful 18-month External Parallel Run demonstrates these operational challenges are manageable in practice, but the comparison presented here reflects idealized performance of both methodologies.
...
Generation Shift Key methodology introduces systematic uncertainty. The proportional GSK employed in this thesis represents one of several valid approaches. Literature suggests alternative strategies (merit-order based, capacity-weighted, hybrid) can yield PTDF variations of 10--20\%~\cite{schonheit2020gsk}, which would translate to proportional uncertainty in estimated FBMC benefits. The 6.5\% winter savings should therefore be interpreted as approximately 6.5\% ± 1.3\% (assuming linear propagation of 20\% PTDF uncertainty), though the actual uncertainty may be non-linear.

The GSK-dependent uncertainty has concrete implications for interpreting results. If a merit-order GSK were employed instead of the proportional approach, the estimated winter savings might range from approximately 5.2\% to 7.8\% rather than the point estimate of 6.5\%. However, the directional findings---that FBMC outperforms ATC, that benefits concentrate in winter, and that benefits scale with congestion---are expected to be robust across GSK methodologies, as these patterns reflect fundamental differences between flow-based and NTC-based constraint formulations rather than specific parameter values.

While the analysis demonstrates strong association between transmission congestion and FBMC benefits, and Scenario C confirms that price convergence eliminates benefits, the research design does not fully isolate congestion as the sole causal driver. Winter periods exhibit multiple correlated characteristics: high congestion, large price spreads, elevated demand variability, and different generation mix composition. The sensitivity scenarios tested price variations (Scenarios A, B) and capacity constraints (Scenario D) separately, but did not systematically isolate conditions such as high congestion with small price spreads. While theoretical reasoning and the price convergence test strongly support the arbitrage facilitation mechanism, a fully controlled experimental design would require additional scenarios disentangling these correlated factors. The practical implication remains clear: FBMC delivers greatest value during winter conditions when congestion, price spreads, and demand stress coincide.
...
The central finding is that FBMC provides measurable and economically significant efficiency gains over ATC, but these gains are neither uniform nor guaranteed. Under winter conditions with high north-south congestion, FBMC achieves cost savings of approximately 6.5\% compared to ATC (subject to GSK-dependent uncertainty of ±1--2 percentage points)---a substantial improvement that validates the Nordic region's decision to implement flow-based allocation. However, during summer periods with low demand and minimal congestion, FBMC benefits shrink to just 0.25\%, with benefits varying by more than an order of magnitude between seasons. This pattern reveals that FBMC is not a universal improvement but rather a tool whose value is contingent on system conditions.

The corridor isolation experiment quantified the physical mechanisms: approximately 32\% of FBMC benefits arise from loop flow exploitation through Norwegian interconnections, while 68\% derive from improved internal corridor optimization. This demonstrates that FBMC delivers value through complementary mechanisms rather than depending solely on network loops, with cross-border connections providing amplification rather than being a prerequisite. This finding has implications for transmission planning---while investments that create network loops yield compounding benefits, FBMC delivers positive welfare effects even in predominantly radial topologies.
...
\textbf{Distributional equity analysis:} This thesis quantifies aggregate welfare gains but does not examine how benefits distribute across market participants. Future research should investigate whether FBMC savings accrue primarily to industrial consumers in southern Sweden (SE3--SE4) or diffuse broadly to residential customers, and whether northern generation-rich regions (SE1--SE2) capture proportional benefits. Such analysis would inform policy discussions about whether complementary measures are needed to ensure equitable distribution of market coupling benefits.
