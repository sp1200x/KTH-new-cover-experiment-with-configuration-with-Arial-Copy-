...
The distinction lies in the analytical objective. Transmission System Operators require high-resolution nodal models to ensure operational security: managing voltage profiles, satisfying N-1 contingency constraints, and coordinating real-time redispatch. These considerations demand granular representation of individual substations, transformers, and transmission lines.

This thesis addresses a different question: the economic efficiency of capacity allocation methodologies at the market coupling level. For this purpose, zonal aggregation is not merely acceptable---it is standard practice in the academic literature on FBMC analysis~\cite{weinhold_pomato_2021, schonheit_gsk_2020}.

The validity of zonal aggregation rests on whether the model captures the relevant physical mechanisms. The key finding of this thesis---that FBMC benefits arise from exploiting loop flows through cross-border paths---depends on representing the SE2--NO4--NO3--NO1--SE3 alternative routing correctly. This path is explicitly included in the 13-node topology. Intra-zonal constraints, while relevant for TSO operations, do not affect the inter-zonal arbitrage mechanism that drives the welfare differences between FBMC and ATC.

Furthermore, the close correspondence between simulated savings (€4.37M/week) and Nordic RCC official estimates (€6.1M/week average) provides empirical validation that zonal aggregation captures the economically significant dynamics. A model missing critical network features would not reproduce results of the correct magnitude.

The limitation of zonal aggregation is acknowledged: the model cannot identify which specific internal lines constrain flows, nor can it capture locational marginal pricing effects within zones. These limitations are appropriate given the research questions, which focus on inter-zonal capacity allocation rather than intra-zonal congestion management.
...
This discrepancy is consistent with known model simplifications:
\begin{itemize}
    \item The simulation uses static NTC values, whereas operational ATC varies hourly based on TSO security assessments
    \item The model includes only 13 CNECs compared to the hundreds monitored in operational FBMC
    \item The EPR average spans multiple seasons and congestion regimes
\end{itemize}
